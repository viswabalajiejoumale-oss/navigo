...
const DEMO_EXPENSES = [
  { id: "1", mode: "taxi", from: "Home", to: "Office", cost: 250, date: new Date("2024-01-15") },
  { id: "2", mode: "metro", from: "Office", to: "Mall", cost: 40, date: new Date("2024-01-15") },
  { id: "3", mode: "bus", from: "Mall", to: "Home", cost: 25, date: new Date("2024-01-15") },
  { id: "4", mode: "taxi", from: "Home", to: "Airport", cost: 650, date: new Date("2024-01-14") },
  { id: "5", mode: "metro", from: "Station", to: "Office", cost: 45, date: new Date("2024-01-14") },
];
...
const ExpenseTracker = () => {
  ...
  const allExpenses = [...DEMO_EXPENSES, ...state.expenses];
  
  const totalSpent = allExpenses.reduce((sum, e) => sum + e.cost, 0);
  const taxiSpent = allExpenses.filter((e) => e.mode === "taxi").reduce((sum, e) => sum + e.cost, 0);
  const metroSpent = allExpenses.filter((e) => e.mode === "metro").reduce((sum, e) => sum + e.cost, 0);
  const busSpent = allExpenses.filter((e) => e.mode === "bus").reduce((sum, e) => sum + e.cost, 0);

  // Calculate potential savings
  const potentialSavings = Math.round(taxiSpent * 0.7);

  const handleAddExpense = () => {
    if (newExpense.mode && newExpense.from && newExpense.to && newExpense.cost) {
      dispatch({
        type: "ADD_EXPENSE",
        payload: {
      ...
      <main className="container max-w-2xl px-4 py-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-body-md text-muted-foreground">
                You spent ₹{taxiSpent} on Taxis this week. Taking the Metro instead would save you approximately ₹{potentialSavings}!
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-3">
            {[
              { mode: "taxi", label: "Taxi", amount: taxiSpent, color: "bg-transport-taxi" },
              { mode: "metro", label: "Metro", amount: metroSpent, color: "bg-transport-metro" },
              { mode: "bus", label: "Bus", amount: busSpent, color: "bg-transport-bus" },
            ].map((item) => {
                ...
                <div key={item.mode} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-body-sm font-semibold">₹{item.amount}</span>
                    </div>
