...
const BookingDialog = ({
  ...
  const [step, setStep] = useState<
    "details" | "schedule" | "seats" | "payment" | "otp" | "confirmed"
  >("details");
  ...
  useEffect(() => {
    if (pickup && dropoff) {
      ...
      else if (modeName.toLowerCase().includes("train"))
        baseFare = baseDistance * 1.5;
      else if (modeName.toLowerCase().includes("metro"))
        baseFare = baseDistance * 2.5;
      else if (modeName.toLowerCase().includes("taxi"))
        baseFare = baseDistance * 15;
      else if (modeName.toLowerCase().includes("flight"))
        baseFare = baseDistance * 50;
  ...
  const canProceedToSchedule =
    pickup.trim() && dropoff.trim() && phone.trim().length >= 10;
  ...
  const handleVerify = () => {
    if (otp.trim() === generatedOtp) {
      const nextBookingId = `NV${Math.floor(100000 + Math.random() * 900000)}`;
      const nextQr = `QR-${Date.now().toString(36).toUpperCase()}`;
      ...
      localStorage.setItem(
        "navigo_usual_commute",
        JSON.stringify(usualCommute),
      );
      localStorage.setItem(
        "navigo_last_ticket",
        JSON.stringify({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={`sm:max-w-2xl transition-all duration-300 ${isAnimating ? "opacity-50 scale-95" : "opacity-100 scale-100"}`}
      >
        <DialogHeader>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-green-600">
                        {distance} km • {parseInt(passengers)} passenger(s)
                      </p>
                    </div>
            ...
            <Card className="border-blue-200 bg-gradient-to-br from-blue-50 to-white">
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-blue-700 font-medium mb-2 block">
                      Travel Date
                    </label>
                    <Input
                      type="date"
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2 text-amber-800">
                  ...
                  <p className="text-sm font-medium">
                    Seat allocation is subject to availability at the time of
                    booking
                  </p>
                </div>
              </CardContent>
            ...
            <Card className="border-green-300 bg-gradient-to-br from-green-50 via-green-100 to-blue-50">
              <CardContent className="p-8 text-center">
                ...
                <div className="bg-white/80 backdrop-blur rounded-lg p-4 text-left space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600">QR Ticket:</span>
                    <span className="font-semibold text-blue-900">
                      {qrCode}
                    </span>
                  </div>
